...
class PaynowService:
    ...
    def create_express_checkout(self, method: str, amount: Decimal, reference: str,
                               email: str, description: str, token: str = None,
                               phone: str = None, account_number: str = None) -> Dict:
        """
        Create an express checkout payment (no redirect, payment processed in-app)
        
        Supports:
        - Visa/Mastercard (method='vmc', requires token)
        - EcoCash (method='ecocash', requires phone)
        - OneMoney (method='onemoney', requires phone)
        - InnBucks (method='innbucks', requires account_number)
        - Zimswitch (method='zimswitch')
        
        Args:
            method: Payment method (vmc, ecocash, onemoney, innbucks, zimswitch)
            amount: Payment amount
            reference: Unique payment reference
            email: Customer email
            description: Payment description
            token: Card token (required for vmc)
            phone: Phone number (required for ecocash, onemoney)
            account_number: Account number (required for innbucks)
            
        Returns:
            Dict with success status and payment details
        """
        if not self.client or not self.express_checkout_url:
            return {
                "success": False,
                "error": "Paynow not configured"
            }
        ...
        if method == 'vmc' and not token:
            return {
                "success": False,
                "error": "Card token required for Visa/Mastercard express checkout"
            }
        
        if method in ['ecocash', 'onemoney'] and not phone:
            return {
                "success": False,
                "error": f"Phone number required for {method} express checkout"
            }
        
        if method == 'innbucks' and not account_number:
            return {
                "success": False,
                "error": "Account number required for InnBucks express checkout"
            }
        ...
        try:
            ...
            if method == 'vmc':
                data['token'] = token
            elif method in ['ecocash', 'onemoney']:
                data['phone'] = phone
            ...
            if response.status_code == 200:
                ...
                if response_data.get('status') == 'Ok':
                    ...
                    return {
                        "success": False,
                        "error": response_data.get('error', 'Express checkout failed'),
                        "reference": reference
                    }
            else:
                return {
    ...
    def tokenize_card(self, card_number: str, cardholder_name: str, expiry_month: str,
                     expiry_year: str, cvv: str) -> Dict:
        """
        ...
        try:
            ...
            tokenize_url = "https://www.paynow.co.zw/interface/tokenize"
            response = requests.post(
                tokenize_url,
                data=data,
                timeout=30
            ...
            if response.status_code == 200:
                ...
                if response_data.get('status') == 'Ok':
                    ...
                    return {
                        "success": False,
                        "error": response_data.get('error', 'Card tokenization failed')
                    }
            else:
                return {
                    "success": False,
